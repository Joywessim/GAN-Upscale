...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, batch_norm=False, activation=None):
        ...
        if activation and activation.lower() in activation_dict:
            layers.append(activation_dict[activation.lower()])
...
class SubPixelConvBlock(nn.Module):
    """
    Sub-pixel convolutional block for upscaling images.
    """
...
class ResBlock(nn.Module):
    """
    Residual block with two convolutional blocks.
    """
    def __init__(self, channels):
        super(ResBlock, self).__init__()
        self.conv1 = ConvBlock(channels, channels, kernel_size=3, batch_norm=True, activation='prelu')
        self.conv2 = ConvBlock(channels, channels, kernel_size=3, batch_norm=True, activation=None)
...
class SRResNet(nn.Module):
    ...
    """
    def __init__(self, in_channels=3, base_channels=64, num_blocks=16, upscale_factor=4):
        super(SRResNet, self).__init__()
        self.input_conv = ConvBlock(in_channels, base_channels, kernel_size=9, batch_norm=False, activation='prelu')
        self.res_blocks = nn.Sequential(*[ResBlock(base_channels) for _ in range(num_blocks)])
        self.inter_conv = ConvBlock(base_channels, base_channels, kernel_size=3, batch_norm=True, activation=None)
        self.upscale_blocks = nn.Sequential(*[SubPixelConvBlock(base_channels, upscale_factor=2) for _ in range(upscale_factor//2)])
        self.output_conv = ConvBlock(base_channels, in_channels, kernel_size=9, batch_norm=False, activation='tanh')
